...
%
% Näheres zur Lizenz entnehmen Sie der Datei LICENSE.
...
\newcommand{\myDeclarationDate}{\mySubmissionDate}

%
% 2. Hat diese Arbeit einen Sperrvermerk?
%
\newboolean{lockflag}
...
% Zeilenabstand (nicht identisch mit MS Word Abständen)
% Für 1,2-fach je nach Schriftgröße etwa:
% 10pt -> 1.2
% 11pt -> 1.24
% 12pt -> 1.21
\usepackage{setspace}
...
\usepackage{listings}
\lstset{ %
	basicstyle=\small\linespread{1}\ttfamily,	% Schriftgröße/-style im Codeblock
	...
	frame=tb,				% Rahmen um Codeblock (Standard: „single“)
	framesep=8pt,
	xleftmargin=18pt,
	numbersep=8pt,
	...
	showstringspaces=false, % hide weird space character in strings
...
\newcommand{\imgABCcontinued}[3]{%
	...
	%%  nur „t“ oder „tb“ statt „htbp“)
		...
		\begin{subfigure}[t]{0.3\textwidth}%
			\centering%
			\includegraphics[width=\tmpvalfou]{\tmpvalfiv}%
			\caption{\tmpvalsix}\label{fig:\tmpvalone-A}%
		\end{subfigure}%
		\hfill~%
		\begin{subfigure}[t]{0.3\textwidth}%
			\centering%
			\includegraphics[width=\tmpvalsev]{\tmpvaleig}%
...
\renewcommand{\mtcgapbeforeheads}{0pt} % 0
\renewcommand{\mtcgapafterheads}{-40pt} % -40
...
\WarningFilter{minitoc(hints)}{W0075}
\WarningFilter{minitoc(hints)}{W0076}
\WarningFilter{minitoc(hints)}{W0077}
...
% For special unicode characters such as \warning
...
\hypersetup{%
	...
	pdfauthor       = {\myName},
	pdftitle        = {{\myTitle}},
	pdfsubject      = {Belegarbeit an der Berufsakademie Sachsen, \myAcademy},
...
\renewcommand*{\backrefalt}[4]{%
	\ifcase #1 %
	\or
		\\(referenziert auf Seite~#2)%
	\else
		\\(referenziert auf Seiten~#2)%
	\fi%
}
